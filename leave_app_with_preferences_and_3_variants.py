...
# ── email confirmation handler (runs before auth wall) ────────────────────────
qp = st.query_params
...
if leave_all.empty:
    ...
    show_cols = [c for c in ["consultant_name","requester_email","start_date","end_date",
                              "leave_type","notes","updated_at"] if c in leave_all.columns]
    st.dataframe(leave_all[show_cols], hide_index=True, use_container_width=True)

# ── admin: all preferred shifts ───────────────────────────────────────────────
st.subheader("All preferred shift requests")
if prefs_all.empty:
    st.info("No preferred shift requests in the system.")
else:
    pref_cols = [c for c in ["consultant_name","requester_email","start_date","end_date",
                              "pref_kind","shift_type","weight","notes","updated_at"] if c in prefs_all.columns]
    st.dataframe(prefs_all[pref_cols], hide_index=True, use_container_width=True)
