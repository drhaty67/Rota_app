...
# ── password recovery handler (token_hash flow) ──────────────────────────────
# When the user clicks the reset link in their email, they arrive with
# ?token_hash=...&type=recovery. We verify the token and let them set a new password.
qp = st.query_params
if qp.get("type") == "recovery" and (qp.get("token_hash") or qp.get("code")):
    st.title("Reset your password")

    # If password was already reset successfully this session
    ...
    # Verify the token_hash to get an authenticated session (only once)
    if "reset_session_token" not in st.session_state:
        try:
            client = base_client()
            token_hash = qp.get("token_hash") or qp.get("code")
            resp = client.auth.verify_otp({
                "token_hash": token_hash,
                "type": "recovery",
            })
            if resp and resp.session:
                st.session_state["reset_session_token"] = resp.session.access_token
                st.session_state["reset_refresh_token"] = resp.session.refresh_token
            else:
            ...
            err_msg = str(e).lower()
            if "expired" in err_msg or "invalid" in err_msg or "not found" in err_msg:
                st.error("The reset link has expired or is invalid. "
                         "Please request a new password reset from the sign-in page.")
            else:
                st.error("Failed to verify reset link.")
                st.exception(e)
            if st.button("Go to Sign In", use_container_width=True):
                st.query_params.clear()
                st.rerun()
    ...
    if submitted:
        if not new_password or not confirm_password:
            ...
            try:
                client = base_client()
                # Re-establish the authenticated session from the recovery token
                client.auth.set_session(
                    st.session_state["reset_session_token"],
                    st.session_state["reset_refresh_token"]
                )
                client.auth.update_user({"password": new_password})
                st.session_state["password_reset_done"] = True
                st.session_state.pop("reset_session_token", None)
...
# ── email confirmation handler (signup) ───────────────────────────────────────
...
def fetch_pre_allocs(period_name: str) -> pd.DataFrame:
    """Fetch saved pre-allocations for a given rota period from Supabase."""
    try:
        r = db.table("pre_allocations").select("*").eq("period_name", period_name).execute()
    except Exception:
        return pd.DataFrame()
    df = pd.DataFrame(r.data or [])
...
def save_pre_alloc(period_name: str, consultant_name: str, week_start, block_type: str):
    """Insert a pre-allocation into Supabase."""
    db.table("pre_allocations").insert({
...
def delete_pre_alloc(alloc_id):
    """Delete a pre-allocation by its Supabase row id."""
    db.table("pre_allocations").delete().eq("id", alloc_id).execute()
...
# ── Pre-populate shifts UI (persisted to Supabase) ────────────────────────
st.markdown("**Pre-populate shifts (optional)**")
st.caption("Specify shifts you want to lock in before the solver runs. "
           "These are saved and will persist across redrafts.")

saved_pre_allocs = fetch_pre_allocs(period_name)
...
with st.expander("Add a pre-allocated shift", expanded=False):
    ...
    if pa_submit and pa_name.strip():
        try:
            save_pre_alloc(period_name, pa_name.strip(), pa_week, pa_block)
            st.success(f"Saved: {pa_name.strip()} → {pa_block} (week of {pa_week})")
            st.rerun()
        except Exception as e:
            st.error("Failed to save pre-allocation.")
            st.exception(e)

if not saved_pre_allocs.empty:
    st.markdown("**Current pre-allocations:**")
    for _, pa in saved_pre_allocs.iterrows():
        col_pa, col_del = st.columns([4, 1])
        with col_pa:
            st.write(f"• {pa['consultant_name']} → {pa['block_type']} (week of {pa['week_start']})")
        with col_del:
            if st.button("Remove", key=f"rm_pa_{pa['id']}"):
                try:
                    delete_pre_alloc(pa["id"])
                    st.rerun()
                except Exception as e:
...
relax_targets         = st.checkbox("Allow fallback: relax allocation targets (±1 block per consultant)", value=True)
...
if template is None:
    ...
    if st.button("Draft 3 rota versions", key="draft_run", type="primary"):
        ...
        with st.spinner("Running solver for 3 variants…"):
            ...
            pre_allocs_db = fetch_pre_allocs(period_name)
            if not pre_allocs_db.empty:
                ...
                for _, pa in pre_allocs_db.iterrows():
            ...
            if relax_targets:
                attempts_base.append(("Relax both + targets", ["--no_hard_week_gap","--no_hard_no_consec_weekends","--relax_targets"]))
            if relax_targets and not (relax_week_gap and relax_no_consec_wknds):
                attempts_base.append(("Relax targets only", ["--relax_targets"]))
