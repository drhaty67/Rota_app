...
# ── password recovery handler (PKCE code flow) ───────────────────────────────
# When the user clicks the reset link in their email, they arrive with
# ?code=...&type=recovery. We handle the reset inline before anything else.
qp = st.query_params
if qp.get("type") == "recovery" and qp.get("code"):
    ...
    # Exchange the code for a session (only once)
    if "reset_session_token" not in st.session_state:
        try:
            client = base_client()
            resp = client.auth.exchange_code_for_session({"auth_code": qp["code"]})
