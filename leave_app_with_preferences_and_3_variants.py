...
# Streamlit cannot read fragments, so this JS snippet detects them and converts
# to query parameters, then reloads the page so Python can read them.
...
(function() {
    ...
    if (hash && hash.length > 1) {
        ...
        if (accessToken && refreshToken) {
            ...
            if (tokenType) url.searchParams.set('type', tokenType);
            window.parent.location.href = url.toString();
        } else if (error) {
            // Convert error fragment to query params so Python can display it
            const url = new URL(window.parent.location.href.split('#')[0]);
...
"""
components.html(_FRAGMENT_JS, height=0, width=0)

# ── handle auth errors from fragment (e.g. expired OTP) ──────────────────────
qp = st.query_params
if "auth_error" in qp:
    error_desc = qp.get("error_description", "An authentication error occurred.")
    st.error(f"Password reset failed: {error_desc}")
    st.info("Please request a new password reset link from the sign-in page.")
