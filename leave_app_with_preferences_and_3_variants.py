...
if len(results) >= 2:
    ...
    if not sheets:
        ...
        try:
            ...
            diffs = pd.DataFrame()  # keep type consistent with helper (DataFrame)
            s = {"changed_cells": 0}

        c1, c2, c3 = st.columns(3)

        # diffs may be a DataFrame (variant_diff_helpers) or a list (fallback). Handle both safely.
        diffs_is_df = hasattr(diffs, "empty") and hasattr(diffs, "head")
        diffs_len = int(len(diffs.index)) if diffs_is_df else int(len(diffs))

        c1.metric("Changed cells", s.get("changed_cells", diffs_len))
        c2.metric("Baseline", baseline_name)
        c3.metric("Compared", compare_name)

        if diffs_is_df:
            no_diffs = diffs.empty
        else:
            no_diffs = (diffs_len == 0)
