...
def solve(
    ...
    for i, c in enumerate(consultants):
        ...
        if c.name.strip().lower() in FLEX_NAMES:
            ...
            # Allow flexible range, capped by actual availability
            effective_min = min(max(0, targets[i] - 2), avail_w)
            effective_max = min(targets[i] + 1, avail_w)
            model.Add(total_alloc >= effective_min)
            model.Add(total_alloc <= effective_max)
            dev = model.NewIntVar(0, targets[i] + 1, f"tdev_{i}")
    ...
    for i, c in enumerate(consultants):
        ...
        for w, wk in enumerate(weeks):
            ...
            if can_any:
                available_weeks += 1
        min_target = max(0, targets[i] - 1) if relax_targets else targets[i]
