...
def solve(
    ...
    # Mandatory blocks: exactly one consultant OR vacancy
    # Optional blocks: at most one consultant
    # ===================================================================
    for w in range(W):
        for bt in MANDATORY_BLOCKS:
            model.Add(
    ...
    for i, c in enumerate(consultants):
        ...
        if is_flex:
            # Vacancy/flex: at most target, penalise under-use
    ...
    for i, c in enumerate(consultants):
        ...
        if c.wte == 1.0 and not c.block_rules:
            ...
            # No optional blocks unless eligible and somehow needed
            for bt in OPTIONAL_BLOCKS:
                model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            ...
            # >= 1 WTE no SC (treat same as 1 WTE): one each mandatory
            for bt in MANDATORY_BLOCKS:
                if bt in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 1)
                else:
