...
def solve(
    ...
    for i, c in enumerate(consultants):
        ...
        if c.wte < 1.0:
            # Each rule defines exactly how many from that block group
            for rule in c.block_rules:
                # Only include blocks that are in this consultant's allowed set
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible:
                    continue
                group_sum = sum(x[(w, b, i)] for w in range(W) for b in eligible)
                if rule["fractional"]:
                    # e.g. old "1/2 D" legacy — treat as at-most constraint
                    model.Add(group_sum <= rule["count"])
                else:
                    # Hard: exactly this many from this group across the cycle
                    model.Add(group_sum == rule["count"])
        else:
            # >= 1 WTE: rules describe extras; each group must appear at least once
            for rule in c.block_rules:
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible:
                    continue
