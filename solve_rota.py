...
def solve(
    ...
    # -----------------------------------------------------------------------
    # Allocation model: exactly one of each allowed block type per consultant.
    # Vacancies fill any slot that can't be covered.
    # -----------------------------------------------------------------------
    ...
    # A vacancy fires when no real consultant can fill a slot.
    # Vacancies are heavily penalised in the objective to minimise their use.
    ...
    # DMonTue and DWedThu are OPTIONAL half-D blocks (at most 1 per week).
    for w in range(W):
    ...
    # -----------------------------------------------------------------------
    # EXACTLY ONE OF EACH ALLOWED TYPE per consultant (at most 1, no duplicates)
    # -----------------------------------------------------------------------
    # Each consultant gets AT MOST 1 of each block type across the cycle.
    # This prevents the solver giving e.g. 2 AB1 and 0 DMonThu.
    # Combined with the weekly coverage constraint (consultant + vacancy == 1),
    # slots are always filled — vacancies absorb any shortfall from leave/gaps.
    #
    # For >= 1 WTE with SC rules (extras): the rule group can get > 1 total
    # across its member block types (e.g. AB1=1, AB2=1, DMonThu=1 from base,
    # plus extras distributed across the same types via group_sum >= ...).
    # -----------------------------------------------------------------------
    for i, c in enumerate(consultants):
        if c.block_rules and c.wte < 1.0:
            # Part-time with rules: enforce each rule's group count
            for rule in c.block_rules:
                ...
                if rule["fractional"]:
                    ...
                    model.Add(group_sum >= scaled)
            # Also enforce no duplicates: at most 1 per individual block type
            for b in allowed_blocks[i]:
                model.Add(sum(x[(w, b, i)] for w in range(W)) <= 1)
        elif c.block_rules and c.wte >= 1.0:
            # >= 1 WTE with SC rules: at most 1 of each base mandatory type
            for b in MANDATORY_BLOCKS:
                if b in allowed_blocks[i]:
                    model.Add(sum(x[(w, b, i)] for w in range(W)) <= 1)
            # SC rule group: at least the extra count from that group
            for rule in c.block_rules:
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible:
                    continue
                scaled = max(1, round(rule["count"] * W / REF_WEEKS))
                group_sum = sum(x[(w, b, i)] for w in range(W) for b in eligible)
                # The base gives 1 of each type already (up to <= 1 each).
                # Extras go on top — allow the group sum to exceed the base.
                model.Add(group_sum >= 1 + scaled)
            # For the extra rules, the group_sum >= 1+scaled may require > 1
            # of some block type. Relax the <= 1 for rule-eligible blocks:
            for rule in c.block_rules:
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                for b in eligible:
                    # Allow up to 2 (base 1 + 1 extra) for blocks in rule groups
                    model.Add(sum(x[(w, b, i)] for w in range(W)) <= 1 + len([
                        r for r in c.block_rules
                        if b in [bb for bb in r["blocks"] if bb in allowed_blocks[i]]
                    ]))
        else:
            # No SC rules: at most 1 of each allowed block type (no duplicates)
            for b in allowed_blocks[i]:
                model.Add(sum(x[(w, b, i)] for w in range(W)) <= 1)
