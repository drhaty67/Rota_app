...
def solve(
    ...
    # -----------------------------------------------------------------------
    # EXACTLY ONE OF EACH ALLOWED TYPE per consultant
    # -----------------------------------------------------------------------
    # A 1 WTE consultant with allowed=[AB1,AB2,DMonThu,WeekendAB,WeekendMixed]
    # gets exactly 1 AB1, 1 AB2, 1 DMonThu, 1 WeekendAB, 1 WeekendMixed.
    # A <1 WTE consultant with SC rules gets exactly what the rules specify.
    # A >1 WTE consultant with SC rules gets 1 of each base type PLUS extras.
    # If a consultant can't be assigned a type due to leave, the vacancy fills it.
    # -----------------------------------------------------------------------
        ...
        if c.block_rules and c.wte < 1.0:
            ...
            for rule in c.block_rules:
                ...
                if rule["fractional"]:
                    ...
                    # Exactly this many (one of each type in the group)
                    model.Add(group_sum == scaled)
        elif c.block_rules and c.wte >= 1.0:
            # >= 1 WTE with rules: exactly 1 of each base mandatory type
            # PLUS extra allocations from the rules
            for b in MANDATORY_BLOCKS:
                if b in allowed_blocks[i]:
                    model.Add(sum(x[(w, b, i)] for w in range(W)) == 1)
            # Extra allocations from SC rules
            for rule in c.block_rules:
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible:
                    continue
                scaled = max(1, round(rule["count"] * W / REF_WEEKS))
                group_sum = sum(x[(w, b, i)] for w in range(W) for b in eligible)
                # Must have at least (1 base + extra) across eligible blocks
                model.Add(group_sum >= 1 + scaled)
        else:
            # No SC rules: exactly 1 of each allowed block type
