...
def solve(
    ...
    for i, c in enumerate(consultants):
        if c.wte == 1.0 and not c.block_rules:
            # 1 WTE no SC: at most one of each mandatory type, no half-D
            # The target constraint (C8) ensures they get 5 full blocks total.
            for bt in ["AB1", "AB2", "WeekendAB", "WeekendMixed", "DMonThu"]:
                if bt in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) <= 1)
                else:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            # No half-D for standard 1 WTE
            for bt in HALF_D_TYPES:
                model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            ...
            # < 1 WTE no SC: at most 1 of each type, no half-D
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
                elif bt in HALF_D_TYPES:
                    # Block half-D for non-SC
                    if i not in sc_allows_half_d:
                        model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
                    else:
                        model.Add(sum(x[(w, bt, i)] for w in range(W)) <= 1)
