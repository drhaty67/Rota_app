...
def _target_allocations(c: Consultant) -> Tuple[int, List[str]]:
    """Return (target_count, allowed_block_types) for a consultant.

    Special circumstances rules (col G) are interpreted differently by WTE:

    < 1.0 WTE with rules:
        The rules ARE the complete allocation spec.
        target = sum of rule counts (after eligibility filtering).
        allowed = union of eligible block types across all rules.

    >= 1.0 WTE with rules:
        The rules define EXTRA allocations on top of the standard 5.
        target = 5 + sum of rule counts.
        allowed = all 5 standard block types PLUS any extra types from rules.

    No rules (col G empty):
        < 1.0 WTE -> round(wte * 5) blocks; drop from BLOCK_DROP_ORDER.
        >= 1.0 WTE -> 5 blocks, all eligible types.
    """
    ...
    if c.block_rules:
        if c.wte < 1.0:
            # Rules are the complete spec for part-time consultants
            allowed_set: set = set()
            target = 0
            ...
            # Rules are extras on top of the standard 5 for >= 1 WTE
            extra_target = 0
            ...
            return 5 + extra_target, allowed

    # No rules: default WTE-based logic (half-D types never used here)
    base = 5
    if c.wte < 1.0:
        target = max(1, round(c.wte * base))
        allowed = list(DEFAULT_BLOCK_TYPES)
        to_drop = base - target
    ...
    return target, _apply_eligibility(allowed)
...
def solve(
    ...
    # Per-consultant allocation targets and allowed block types
    targets = []
    allowed_blocks = []
    for c in consultants:
        t, ab = _target_allocations(c)
        targets.append(t)
    ...
    for i, c in enumerate(consultants):
        ...
        if c.block_rules and c.wte < 1.0:
            # Part-time with explicit rules: enforce each rule's group count exactly.
            # Fractional rules (e.g. "1/2 D") get an upper-bound only (0 or 1 allowed).
            for rule in c.block_rules:
                ...
                if rule["fractional"]:
                    model.Add(group_alloc <= rule["count"])
                else:
                    model.Add(group_alloc == rule["count"])

        elif c.block_rules and c.wte >= 1.0:
            # Full/over-time with rules: enforce base 5 (one of each eligible type)
            # plus the extra rule-based allocations.
            for b in BLOCK_TYPES:
                if b in allowed_blocks[i]:
                    model.Add(sum(x[(w, b, i)] for w in range(W)) >= 1)
            # Extra allocations: enforce each rule's group count as additional
            for rule in c.block_rules:
                eligible_blocks = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible_blocks:
                    continue
                group_alloc = sum(x[(w, b, i)] for w in range(W) for b in eligible_blocks)
                if rule["fractional"]:
                    # e.g. "1.5 D" -> at least 1 extra D beyond the base 1
                    model.Add(group_alloc >= 2)
                else:
                    # e.g. "1 AB1 or AB2 or DMonThu" -> at least 2 total (base 1 + extra 1)
                    model.Add(group_alloc >= 2)

        else:
            # No rules: default behaviour
            if len(allowed_blocks[i]) == targets[i]:
