...
def solve(
    ...
    for i, c in enumerate(consultants):
        ...
        if c.name.strip().lower() in FLEX_NAMES:
            ...
            # Allow target ± 1, penalise deviation
            model.Add(total_alloc >= max(0, targets[i] - 1))
            model.Add(total_alloc <= targets[i] + 1)
            dev = model.NewIntVar(0, targets[i] + 1, f"tdev_{i}")
            model.AddAbsEquality(dev, total_alloc - targets[i])
        ...
        if c.block_rules and c.wte < 1.0:
            ...
            for rule in c.block_rules:
                ...
                if rule["fractional"]:
                    ...
                    if relax_targets:
                        # Soft: allow shortfall with penalty
                        model.Add(group_sum >= max(0, scaled - 1))
                    else:
                        model.Add(group_sum >= scaled)
