...
def solve(
    ...
    targets = [max(1, min(max_per_consultant, round(t))) for t in raw_targets]

    # Adjust rounding errors: if sum(targets) != total_slots, fix the largest remainders
    diff = total_slots - sum(targets)
    if diff != 0:
        remainders = [(raw_targets[i] - targets[i], i) for i in range(N)]
    ...
    # -----------------------------------------------------------------------
    # Hard: each consultant gets exactly targets[i] allocations across the cycle.
    # Soft per-type diversity: for consultants with >= 2 allowed block types,
    # we ask for at least 1 of each type (soft via objective, not hard constraint,
    # to avoid over-constraining when leave blocks many weeks).
    # For part-time with explicit SC rules: restrict to their allowed blocks only
    # (already done via allowed_blocks[i]), no additional hard per-type counts.
    # -----------------------------------------------------------------------
    for i in range(N):
