...
def solve(
    ...
    # -----------------------------------------------------------------------
    for i, c in enumerate(consultants):
        # (A) Total allocation count — aim for target, penalise shortfall
    ...
    # Exempt consultants whose names contain "Vacancy" — they can work consecutive weeks.
    if hard_week_gap:
        exempt_names = {"vacancyc", "vacancyca", "rv vacancy"}
        for i in range(N):
            if names[i].strip().lower() in exempt_names:
                continue  # no gap constraint for vacancy consultants
            for w in range(W - 1):
    ...
    # Under-allocation penalty: penalise each missed allocation per consultant
    # This is even heavier than vacancy so the solver fills consultant targets first.
    UNDER_ALLOC_PENALTY = 100000
    under_alloc_terms = []
    for i in range(N):
        actual = sum(x[(w, b, i)] for w in range(W) for b in BLOCK_TYPES)
        shortfall = model.NewIntVar(0, targets[i], f"short_{i}")
        model.Add(shortfall == targets[i] - actual)
        under_alloc_terms.append(UNDER_ALLOC_PENALTY * shortfall)
    under_alloc_cost = sum(under_alloc_terms)

    model.Minimize(under_alloc_cost + vacancy_cost + 3 * sum(devBH) + 2 * sum(devW) + cardiac_penalty - pref_reward)
