...
def read_inputs(path: str, override_start: Optional[date] = None, override_end: Optional[date] = None):
    ...
    # Case-insensitive lookup: maps lowercased name -> canonical consultant name
    _name_lookup: Dict[str, str] = {c.name.strip().lower(): c.name for c in consultants}

    for r in range(2, 5000):
        nm = lws[f"A{r}"].value
        if not nm:
            continue
        # Load ALL leave/NOC entries regardless of approval status (col E).
        # All absence types (Annual, Study, NOC) are treated equally —
        # no shifts should be allocated during any absence period.
        s = excel_date(lws[f"B{r}"].value)
        e = excel_date(lws[f"C{r}"].value)
        if not s or not e:
            continue
        nm_raw = str(nm).strip()
        # Resolve name: exact match first, then case-insensitive
        if nm_raw in leave_map:
            resolved = nm_raw
        elif nm_raw.lower() in _name_lookup:
            resolved = _name_lookup[nm_raw.lower()]
        else:
            continue
        for d in daterange(s, e):
            leave_map[resolved].add(d)
...
def solve(
    ...
    """Solve the rota allocation using CP-SAT (half-D = 0.5 model).

    Key design:
      - Full blocks (AB1, AB2, DMonThu, WeekendAB, WeekendMixed) = 2 half-units
      - Half-D blocks (DMonTue, DWedThu) = 1 half-unit
      - No special 'flex' handling — all consultants (including vacancy posts)
        follow the same rules.
      - D-coverage: each week needs DMonThu OR (DMonTue + DWedThu) OR vacancy.
        DMonThu and half-D are mutually exclusive within a week.
      - Half-D restricted to consultants whose SC rules include them.
      - Pairing preference: soft penalty when only one half-D is filled in a week.
    """
    # -----------------------------------------------------------------------
    ...
    HALF_D_TYPES = ["DMonTue", "DWedThu"]
    FULL_BLOCK_TYPES = ["AB1", "AB2", "DMonThu", "WeekendAB", "WeekendMixed"]

    print(f"\n{'='*60}")
    print(f"SOLVER DIAGNOSTICS (half-D = 0.5, simplified)")
    print(f"{'='*60}")
            ...
            for rule in c.block_rules:
                for b in rule["blocks"]:
                    if b in ("DMonTue", "DWedThu"):
                        ...
                        break
        # Vacancy consultants can fill any slot including half-D
        if "vacancy" in c.name.strip().lower():
            sc_allows_half_d.add(i)

    # -----------------------------------------------------------------------
    # Compute targets in HALF-BLOCK UNITS
    # -----------------------------------------------------------------------
    target_half_units: List[int] = []
    consultant_sc_rules: List[Optional[list]] = []

    for i, c in enumerate(consultants):
        if c.wte == 1.0 and not c.block_rules:
            # 1 WTE no SC: 5 full blocks = 10 half-units
            target_half_units.append(10)
            consultant_sc_rules.append(None)
        elif c.wte < 1.0 and c.block_rules:
            # < 1 WTE with SC: sum rules
            total_hu = 0
            for rule in c.block_rules:
                if rule["fractional"]:
                    total_hu += 1
                else:
                    all_half_d = all(b in HALF_D_TYPES for b in rule["blocks"])
                    if all_half_d:
                        total_hu += rule["count"] * 1
                    else:
                        total_hu += rule["count"] * 2
            target_half_units.append(max(1, total_hu))
            consultant_sc_rules.append(list(c.block_rules))
        elif c.wte > 1.0 and c.block_rules:
            # > 1 WTE with SC: base 10 + extras
            base_hu = 10
            extras_hu = 0
            for rule in c.block_rules:
                if rule["fractional"]:
                    ...
                    all_half_d = all(b in HALF_D_TYPES for b in rule["blocks"])
                    if all_half_d:
                        extras_hu += rule["count"] * 1
                    else:
                        extras_hu += rule["count"] * 2
            target_half_units.append(base_hu + extras_hu)
            consultant_sc_rules.append(list(c.block_rules))
        elif c.wte < 1.0 and not c.block_rules:
            # < 1 WTE no SC: round(WTE * 5) full blocks
            full_blocks = max(1, round(c.wte * 5))
            target_half_units.append(full_blocks * 2)
            consultant_sc_rules.append(None)
        else:
            # >= 1 WTE no SC: 5 full blocks = 10 half-units
            target_half_units.append(10)
    ...
    max_possible_hu: List[int] = [av * 2 for av in avail_per_consultant]

    print(f"\nConsultant targets (half-block units) and availability:")
    infeasible_flags: List[str] = []
    for i, c in enumerate(consultants):
        leave_days_in_period = len([dd for dd in leave.get(c.name, set())
                                    if start <= dd <= end])
        flag = ""
        if max_possible_hu[i] < target_half_units[i]:
            flag = " *** IMPOSSIBLE ***"
            infeasible_flags.append(c.name)
        print(f"  {c.name}: WTE={c.wte}, cardiac={c.cardiac}, "
              f"target_hu={target_half_units[i]}, "
              f"avail_weeks={avail_per_consultant[i]}, "
              f"max_hu={max_possible_hu[i]}, "
              f"leave_days={leave_days_in_period}, "
              f"eligible={eligible[i]}{flag}")

    if infeasible_flags:
        print(f"\n*** WARNING: Cannot meet targets: {infeasible_flags}")
        if not relax_targets:
            print(f"    Consider using --relax_targets.")

    print(f"\nSC allows half-D: {[names[i] for i in sc_allows_half_d]}")
    print()
    ...
    # Vacancy variables for each mandatory slot + half-D slots
    vacancy = {}
    for w in range(W):
        for bt in MANDATORY_BLOCKS:
            vacancy[(w, bt)] = model.NewBoolVar(f"vac_{w}_{bt}")
        for bt in HALF_D_TYPES:
            vacancy[(w, bt)] = model.NewBoolVar(f"vac_{w}_{bt}")

    # Soft penalty collector
    soft_penalty_terms = []

    # ===================================================================
    # HARD CONSTRAINT 1: Non-D mandatory slot coverage
    # AB1, AB2, WeekendAB, WeekendMixed: exactly 1 consultant OR vacancy
    # ===================================================================
    NON_D_MANDATORY = ["AB1", "AB2", "WeekendAB", "WeekendMixed"]
    for w in range(W):
        for bt in NON_D_MANDATORY:
            model.Add(
                sum(x[(w, bt, i)] for i in range(N)) + vacancy[(w, bt)] == 1
            )

    # ===================================================================
    # HARD CONSTRAINT 2: D-Coverage and mutual exclusion
    #
    # Each week: D is covered by EITHER:
    #   (a) Exactly 1 DMonThu (full), OR
    #   (b) DMonTue slot + DWedThu slot (each filled by consultant or vacancy)
    #
    # DMonThu and half-D blocks are mutually exclusive within a week.
    # At least one D-type must be used (no week with zero D-coverage unless
    # all go to vacancy).
    # ===================================================================
    for w in range(W):
        dmt_sum = sum(x[(w, "DMonThu", i)] for i in range(N))
        dmtu_sum = sum(x[(w, "DMonTue", i)] for i in range(N))
        dwt_sum = sum(x[(w, "DWedThu", i)] for i in range(N))

        # At most 1 person per D-type
        model.Add(dmt_sum <= 1)
        model.Add(dmtu_sum <= 1)
        model.Add(dwt_sum <= 1)

        # Mutual exclusion: if DMonThu is used, no half-D; if half-D, no DMonThu
        # Use an indicator: has_full_d = 1 if DMonThu assigned or DMonThu vacancy used
        has_full_d = model.NewBoolVar(f"has_full_d_{w}")

        # If has_full_d: DMonThu filled (by person or vacancy), no half-D
        model.Add(dmt_sum + vacancy[(w, "DMonThu")] == 1).OnlyEnforceIf(has_full_d)
        model.Add(dmtu_sum == 0).OnlyEnforceIf(has_full_d)
        model.Add(dwt_sum == 0).OnlyEnforceIf(has_full_d)
        model.Add(vacancy[(w, "DMonTue")] == 0).OnlyEnforceIf(has_full_d)
        model.Add(vacancy[(w, "DWedThu")] == 0).OnlyEnforceIf(has_full_d)

        # If NOT has_full_d: no DMonThu, half-D slots active (each has person or vacancy)
        model.Add(dmt_sum == 0).OnlyEnforceIf(has_full_d.Not())
        model.Add(vacancy[(w, "DMonThu")] == 0).OnlyEnforceIf(has_full_d.Not())
        model.Add(dmtu_sum + vacancy[(w, "DMonTue")] == 1).OnlyEnforceIf(has_full_d.Not())
        model.Add(dwt_sum + vacancy[(w, "DWedThu")] == 1).OnlyEnforceIf(has_full_d.Not())

    # ===================================================================
    # HARD CONSTRAINT 3: One block per consultant per week
    # ===================================================================
    for w in range(W):
        for i in range(N):
    ...
    # ===================================================================
    # HARD CONSTRAINT 4: Leave blocking
    # ===================================================================
    for w, wk in enumerate(weeks):
        for bt in BLOCK_TYPES:
    ...
    # ===================================================================
    # HARD CONSTRAINT 5: Block type eligibility
    # ===================================================================
    for w in range(W):
        for bt in BLOCK_TYPES:
            for i in range(N):
    ...
    # ===================================================================
    # HARD CONSTRAINT 6: Half-D restriction
    # Only consultants whose SC rules include DMonTue/DWedThu can be assigned them.
    # ===================================================================
    for w in range(W):
        for bt in HALF_D_TYPES:
            for i in range(N):
                if i not in sc_allows_half_d:
                    model.Add(x[(w, bt, i)] == 0)

    # ===================================================================
    # HARD CONSTRAINT 7: Pre-allocations
    # ===================================================================
    pre_alloc_fixed: Set[Tuple[int, str, int]] = set()
    name_to_idx = {c.name.strip(): idx for idx, c in enumerate(consultants)}
    name_to_idx_lower = {c.name.strip().lower(): idx
    ...
    if pre_allocations:
        ...
        for pa in pre_allocations:
            ...
            if slot_key in pre_alloc_slots:
                print(f"  [SKIP] {bt} week {wk} already assigned to "
                      f"'{pre_alloc_slots[slot_key]}'.")
                continue
            existing = [(wb, bb) for (wb, bb, ib) in pre_alloc_fixed
                        if wb == w and ib == i]
            if existing:
                print(f"  [SKIP] '{nm}' already pre-allocated "
                      f"{existing[0][1]} in week {wk}.")
                continue
            # Lock it in
    ...
    # ===================================================================
    # HARD CONSTRAINT 8: Allocation targets (half-block units)
    #
    # With 18 weeks the solver can be strict:
    #   - Non-vacancy consultants: exact target enforced as hard constraint.
    #     If relax_targets is set, allow ±2 hu (±1 full block) slack with a
    #     heavy penalty for deviation. This is much tighter than the old ±4.
    #   - Vacancy posts: flexible — only bounded by supply (no hard target).
    # ===================================================================
    target_slack_vars = []

    for i, c in enumerate(consultants):
        is_vacancy = "vacancy" in c.name.strip().lower()

        # Weighted allocation sum in half-units
        hu_alloc = (
            sum(x[(w, bt, i)] * 2 for w in range(W) for bt in FULL_BLOCK_TYPES)
            + sum(x[(w, bt, i)] * 1 for w in range(W) for bt in HALF_D_TYPES)
        )

        effective_target = min(target_half_units[i], max_possible_hu[i])

        if is_vacancy:
            # Vacancies are flexible — no hard target, just bounded by supply
            model.Add(hu_alloc >= 0)
            model.Add(hu_alloc <= max_possible_hu[i])
        elif relax_targets:
            # Non-vacancy with relax: tight ±2 hu slack (±1 full block)
            lo = max(0, effective_target - 2)
            hi = effective_target + 2
            model.Add(hu_alloc >= lo)
            model.Add(hu_alloc <= hi)
            # Heavy penalty for any deviation from exact target
            dev = model.NewIntVar(0, W * 2, f"tgt_dev_{i}")
            model.AddAbsEquality(dev, hu_alloc - effective_target)
            target_slack_vars.append(dev)
        else:
            # Non-vacancy strict: exact target
            model.Add(hu_alloc == effective_target)

    # ===================================================================
    # HARD CONSTRAINT 9: Block type rules per consultant category
    #
    # With 18 weeks we enforce STRICT distribution as hard constraints:
    #   - 1 WTE (no SC): exactly 1 AB1, 1 AB2, 1 DMonThu, 1 WeekendAB,
    #                     1 WeekendMixed (total = 5 full blocks)
    #   - > 1 WTE with SC: same base of exactly 1 each + SC extras
    #   - < 1 WTE with SC: only shifts from SC rules (strict)
    #   - < 1 WTE no SC: proportional target, no hard type distribution
    #   - Vacancies: EXEMPT from all hard distribution constraints
    # ===================================================================
    for i, c in enumerate(consultants):
        is_vacancy = "vacancy" in c.name.strip().lower()

        if is_vacancy:
            # Vacancies are exempt from distribution constraints.
            # They can fill any eligible slot as needed.
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
        ...
        if c.wte == 1.0 and not c.block_rules:
            # -------------------------------------------------------
            # 1 WTE no SC: HARD exactly 1 of each mandatory block type
            # -------------------------------------------------------
            for bt in MANDATORY_BLOCKS:
                if bt in eligible[i]:
            ...
            for bt in HALF_D_TYPES:
                model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            # No optional blocks unless eligible (shouldn't happen for standard 1WTE)
            ...
            # -------------------------------------------------------
            # >= 1 WTE no SC (but not exactly 1.0 — rare edge case):
            # Same as 1 WTE — exactly 1 of each mandatory type.
            # -------------------------------------------------------
            for bt in MANDATORY_BLOCKS:
                if bt in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 1)
            ...
            # -------------------------------------------------------
            # > 1 WTE with SC: base exactly 1 of each mandatory type
            # PLUS additional shifts from SC rules.
            # -------------------------------------------------------
            # Base: exactly 1 of each mandatory block type
            for bt in MANDATORY_BLOCKS:
                if bt in eligible[i]:
                    # Count how many EXTRA of this type the SC rules require
                    sc_extra_for_bt = 0
                    for rule in c.block_rules:
                        if bt in rule["blocks"] and len(rule["blocks"]) == 1:
                            if not rule["fractional"]:
                                sc_extra_for_bt += rule["count"]
                    # Total = base 1 + SC extras specifically for this type
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) >= 1)
                else:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)

            # D-coverage base: at least 1 from D-family
            d_family = [b for b in ["DMonThu", "DMonTue", "DWedThu"]
                        if b in eligible[i]]
            if d_family:
                model.Add(sum(x[(w, b, i)] for w in range(W)
                              for b in d_family) >= 1)

            # SC extras: enforce exact counts for each SC rule group
            ...
            for rule in c.block_rules:
                ...
                if rule["fractional"]:
                    # Fractional: allow 0 or 1
                    group_sum = sum(x[(w, b, i)] for w in range(W)
            ...
            for group_blocks_fs, extra_count in group_extras.items():
                ...
                # Base mandatory blocks in this group
                base_in_group = len([b for b in group_blocks
                                     if b in MANDATORY_BLOCKS])
                # Total required = base (1 each) + extra
                model.Add(group_sum == base_in_group + extra_count)

            # Block half-D if not SC-allowed
            if i not in sc_allows_half_d:
                for bt in HALF_D_TYPES:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            ...
            # -------------------------------------------------------
            # < 1 WTE with SC: ONLY shifts from SC rules, nothing else.
            # Strict enforcement of rule counts.
            # -------------------------------------------------------
            mentioned = set()
            for rule in c.block_rules:
                rule_blocks = [b for b in rule["blocks"] if b in eligible[i]]
            ...
            # Block types NOT mentioned in any SC rule: strictly disallow
            for bt in BLOCK_TYPES:
                if bt not in mentioned:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            ...
            # -------------------------------------------------------
            # < 1 WTE no SC: proportional target from C8, no strict
            # type distribution but block ineligible types.
            # -------------------------------------------------------
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
    ...
    # Each weekday: exactly one of A-slot and D-slot holders is cardiac.
    # -------------------------------------------------------------------
    ...
    if hard_no_consecutive_weekends:
        for i in range(N):
            for w in range(W - 1):
                ...
                wknd_next = (x[(w + 1, "WeekendAB", i)]
                             + x[(w + 1, "WeekendMixed", i)])
    ...
    # -------------------------------------------------------------------
    # TIER 4 (5,000): Block type diversity
    # Penalise getting >1 of the same mandatory type (encourages one-of-each)
    # NOTE: Skip consultants with hard type constraints (1WTE no SC,
    # >=1WTE no SC) since C9 already enforces exact counts.
    # -------------------------------------------------------------------
    for i, c in enumerate(consultants):
        is_vacancy = "vacancy" in c.name.strip().lower()
        if is_vacancy:
            continue  # vacancies are flexible
        if c.block_rules and c.wte < 1.0:
            continue  # SC consultants have their own exact rules in C9
        if c.wte >= 1.0 and not c.block_rules:
            continue  # already has hard == 1 constraint in C9
        for bt in MANDATORY_BLOCKS:
            if bt in eligible[i]:
    ...
    # -------------------------------------------------------------------
    # TIER 4b (10,000): Half-D pairing preference
    # Prefer weeks where BOTH DMonTue and DWedThu are filled by real
    # consultants (not vacancy). Penalise weeks where only one half is
    # filled by a consultant.
    # -------------------------------------------------------------------
    PAIRING_PENALTY = 10_000
    for w in range(W):
        dmtu_filled = sum(x[(w, "DMonTue", i)] for i in range(N))
        dwt_filled = sum(x[(w, "DWedThu", i)] for i in range(N))
        # If exactly one half-D is consultant-filled, penalise
        unpaired = model.NewBoolVar(f"unpaired_{w}")
        # unpaired=1 when dmtu_filled + dwt_filled == 1 (one filled, one not)
        diff = model.NewIntVar(-1, 1, f"half_d_diff_{w}")
        model.Add(diff == dmtu_filled - dwt_filled)
        abs_diff = model.NewIntVar(0, 1, f"half_d_abs_{w}")
        model.AddAbsEquality(abs_diff, diff)
        # Only penalise if at least one half-D is in use this week
        any_half_d = model.NewBoolVar(f"any_half_d_{w}")
        model.Add(dmtu_filled + dwt_filled >= 1).OnlyEnforceIf(any_half_d)
        model.Add(dmtu_filled + dwt_filled == 0).OnlyEnforceIf(any_half_d.Not())
        # Penalty = abs_diff * PAIRING_PENALTY (only when half-D week is active)
        pair_pen = model.NewIntVar(0, 1, f"pair_pen_{w}")
        model.Add(pair_pen >= abs_diff - (1 - any_half_d))
        model.Add(pair_pen <= abs_diff)
        model.Add(pair_pen <= any_half_d)
        soft_penalty_terms.append(PAIRING_PENALTY * pair_pen)

    # -------------------------------------------------------------------
    # TIER 5 (reward): Preference satisfaction
    # -------------------------------------------------------------------
    ...
    name_to_i_lower = {c.name.strip().lower(): idx
                       for idx, c in enumerate(consultants)}
    ...
    for w, wk in enumerate(weeks):
        for bt in BLOCK_TYPES:
            bh_count[(w, bt)] = sum(1 for dd in _block_days(wk, bt)
                                    if dd in bank_holidays)
    ...
    expected_bh = [int(round(bh_total * (wte_list[i] / sum_wte) * SCALE))
                   for i in range(N)]
    ...
    for i in range(N):
        model.Add(weekend_duty[i] == sum(
            x[(w, "WeekendAB", i)] + x[(w, "WeekendMixed", i)]
            for w in range(W)
        ))

    weekend_total = 2 * W
    expected_wknd = [int(round(weekend_total * (wte_list[i] / sum_wte) * SCALE))
                     for i in range(N)]
    ...
    # ===================================================================
    vacancy_cost = VACANCY_PENALTY * sum(
        vacancy[(w, bt)] for w in range(W)
        for bt in list(MANDATORY_BLOCKS) + HALF_D_TYPES
    )

    target_dev_cost = (TARGET_DEVIATION_PENALTY * sum(target_slack_vars)
                       if target_slack_vars else 0)
    ...
    if status == cp_model.INFEASIBLE:
        ...
        total_demand_hu = sum(target_half_units)
        total_supply_hu = W * (5 * 2 + 2 * 1)  # 5 full slots + 2 half slots per week
        print(f"  Total supply (half-units): {total_supply_hu}")
        print(f"  Total demand (half-units): {total_demand_hu}")
            ...
            for bt in BLOCK_TYPES:
                ...
                for i in range(N):
                    if solver.Value(x[(w, bt, i)]) == 1:
                        ...
                        break
                if not assigned:
                    if (w, bt) in vacancy and solver.Value(vacancy[(w, bt)]) == 1:
                        sol["assignments"][wk][bt] = "Vacancy"

        # Print summary
        print(f"\nAssignment summary (half-block units):")
        for i, c in enumerate(consultants):
            full_count = sum(
                1 for w in range(W) for bt in FULL_BLOCK_TYPES
                if solver.Value(x[(w, bt, i)]) == 1
            )
            half_count = sum(
                1 for w in range(W) for bt in HALF_D_TYPES
                if solver.Value(x[(w, bt, i)]) == 1
            )
            hu_actual = full_count * 2 + half_count * 1
            types_assigned = [
                bt for bt in BLOCK_TYPES
                if sum(solver.Value(x[(w, bt, i)]) for w in range(W)) > 0
            ]
            print(f"  {c.name}: {hu_actual}/{target_half_units[i]} hu "
                  f"({full_count} full + {half_count} half-D), "
                  f"types={types_assigned}")
...
def export_to_excel(input_path: str, output_path: str, sol: Dict,
    ...
    leave_map = {}
    # Case-insensitive lookup for name matching (Leave names may differ in case)
    _export_name_lookup = {k.strip().lower(): k for k in cardiac.keys()}

    for r in range(2, 5000):
        nm = leave_ws[f"A{r}"].value
        if not nm:
            continue
        # Load ALL leave/NOC entries — treat all absence types equally
        ...
        nm_raw = str(nm).strip()
        # Resolve name: exact match first, then case-insensitive
        if nm_raw in _export_name_lookup.values():
            resolved = nm_raw
        elif nm_raw.lower() in _export_name_lookup:
            resolved = _export_name_lookup[nm_raw.lower()]
        else:
            resolved = nm_raw  # keep as-is for non-active consultants
        leave_map.setdefault(resolved, set())
        d = s
        while d <= e:
            leave_map[resolved].add(d)
            d += timedelta(days=1)
