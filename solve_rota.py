...
def solve(
    ...
    print(f"Relaxations: week_gap={not hard_week_gap}, consec_wknds={not hard_no_consecutive_weekends}, targets={relax_targets}")
    ...
    for i, c in enumerate(consultants):
        leave_days_in_period = len([d for d in leave.get(c.name, set()) if start <= d <= end])
        print(f"  {c.name}: WTE={c.wte}, cardiac={c.cardiac}, leave_days={leave_days_in_period}")
    ...
    print(f"\nTargets (blocks to assign per consultant):")
    for i, c in enumerate(consultants):
        print(f"  {c.name}: target={targets[i]}, allowed_blocks={allowed_blocks[i]}")
    print(f"Total target blocks: {sum(targets)}")
    print(f"Total available slots (W * 5 mandatory + W * 2 optional): {W * 5} mandatory, {W * 2} optional")

    # Supply/demand per block type
    print(f"\nSupply/demand per block type:")
    for b in BLOCK_TYPES:
        supply = W  # each block type appears once per week
        eligible_consultants = [c.name for i, c in enumerate(consultants) if b in allowed_blocks[i]]
        demand_sum = sum(1 for i in range(N) if b in allowed_blocks[i])
        print(f"  {b}: supply={supply} slots/period, eligible={demand_sum} consultants: {eligible_consultants}")
    print()
    ...
    # -----------------------------------------------------------------------
    # (A) Total allocation count == target for real consultants (HARD).
    #     Vacancy-role consultants flex with <= target.
    # (B) SC rule enforcement (HARD).
    # (C) Diversity: prefer 1 of each type (SOFT via objective penalty).
    #     No hard <= 1 per type for standard consultants — leave can force
    #     duplicates when all weeks of a type are blocked.
    # -----------------------------------------------------------------------
    FLEX_NAMES = {"vacancyc", "vacancyca", "rv vacancy"}
    DIVERSITY_PENALTY = 20000  # per duplicate (getting 2 of a type instead of 1)
    TARGET_DEVIATION_PENALTY = 100000  # per block shortfall/excess when targets relaxed
    diversity_terms = []
    target_deviation_terms = []

    # Pre-compute available weeks per consultant (how many weeks they can take at least one block)
    ...
    available_weeks_per_consultant = []
    for i, c in enumerate(consultants):
        avail = 0
        for w, wk in enumerate(weeks):
            ...
            for b in allowed_blocks[i]:
                ...
                if not any(d in leave.get(c.name, set()) for d in days):
                    can_any = True
                    break
            if can_any:
                avail += 1
        available_weeks_per_consultant.append(avail)

    for i, c in enumerate(consultants):
        # (A) Total allocation count
        total_alloc = sum(x[(w, b, i)] for w in range(W) for b in BLOCK_TYPES)
        avail_w = available_weeks_per_consultant[i]
        if c.name.strip().lower() in FLEX_NAMES:
            model.Add(total_alloc <= targets[i])
            # Soft incentive to assign flex consultants their fair share
            dev = model.NewIntVar(0, W, f"tdev_flex_{i}")
            model.AddAbsEquality(dev, total_alloc - min(targets[i], avail_w))
            target_deviation_terms.append(TARGET_DEVIATION_PENALTY * dev)
        elif relax_targets:
            # Allow flexible range — no hard minimum, penalise deviation from target
            effective_max = min(targets[i] + 1, avail_w) if avail_w > 0 else 0
            model.Add(total_alloc <= max(effective_max, 1))
            # Soft penalty for deviation from target
            ideal = min(targets[i], avail_w)
            dev = model.NewIntVar(0, W, f"tdev_{i}")
            model.AddAbsEquality(dev, total_alloc - ideal)
            target_deviation_terms.append(TARGET_DEVIATION_PENALTY * dev)
        else:
            model.Add(total_alloc == targets[i])

        if c.block_rules and c.wte < 1.0:
            # Part-time with SC rules: enforce group counts (hard or soft)
            for rule in c.block_rules:
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible:
                    continue
                ...
                if rule["fractional"]:
                    ...
                    if relax_targets:
                        # Soft: allow significant shortfall with penalty
                        model.Add(group_sum >= 0)
                    else:
                        model.Add(group_sum >= scaled)
            # Soft: penalise duplicates within each block type
            for b in allowed_blocks[i]:
                count_b = sum(x[(w, b, i)] for w in range(W))
                excess = model.NewIntVar(0, W, f"dup_{i}_{b}")
                model.Add(excess >= count_b - 1)
                diversity_terms.append(DIVERSITY_PENALTY * excess)

        elif c.block_rules and c.wte >= 1.0:
            # >= 1 WTE with SC rules: compute max per type from extras
            ...
            # Hard cap per type: 1 base + extras (prevents runaway)
            for b in allowed_blocks[i]:
                max_b = 1 + extra_count_per_block.get(b, 0)
                if not relax_targets:
                    model.Add(sum(x[(w, b, i)] for w in range(W)) <= max_b)
                else:
                    # Soft cap: allow +1 above max with penalty
                    model.Add(sum(x[(w, b, i)] for w in range(W)) <= max_b + 1)
            # Soft: penalise getting more than 1 of a non-extra type
            for b in allowed_blocks[i]:
                ideal = 1 + extra_count_per_block.get(b, 0)
                count_b = sum(x[(w, b, i)] for w in range(W))
                excess = model.NewIntVar(0, W, f"dup_{i}_{b}")
                model.Add(excess >= count_b - ideal)
                diversity_terms.append(DIVERSITY_PENALTY * excess)

        else:
            # Standard: soft diversity (prefer 1 of each, allow duplicates if forced)
            for b in allowed_blocks[i]:
                count_b = sum(x[(w, b, i)] for w in range(W))
                excess = model.NewIntVar(0, W, f"dup_{i}_{b}")
                model.Add(excess >= count_b - 1)
    ...
    diversity_cost = sum(diversity_terms) if diversity_terms else 0
    ...
    if pre_allocations:
        # Track (week, block) → consultant to detect duplicates
        ...
        for pa in pre_allocations:
            ...
            if i is None:
                print(f"  [PRE-ALLOC SKIP] Consultant '{nm}' not found.")
                continue
            w = week_to_idx.get(wk)
            if w is None:
                print(f"  [PRE-ALLOC SKIP] Week {wk} not in period for '{nm}'.")
                continue
            if bt not in BLOCK_TYPES:
                print(f"  [PRE-ALLOC SKIP] Block type '{bt}' invalid for '{nm}'.")
                continue
            # Check for conflict with leave
            days = block_days(weeks[w], bt)
            if any(d in leave.get(names[i], set()) for d in days):
                print(f"  [PRE-ALLOC CONFLICT] '{nm}' has leave during {bt} in week {wk} — skipping.")
                continue
            # Check for eligibility conflict
            if bt not in allowed_blocks[i]:
                print(f"  [PRE-ALLOC CONFLICT] '{nm}' not eligible for {bt} — skipping.")
                continue
            # Check for duplicate slot assignment
            slot_key = (w, bt)
            if slot_key in pre_alloc_slots:
                print(f"  [PRE-ALLOC CONFLICT] {bt} week {wk} already assigned to '{pre_alloc_slots[slot_key]}' — skipping '{nm}'.")
                continue
            # Check consultant not already assigned another block this week
            existing_in_week = [(wb, bb) for (wb, bb, ib) in pre_alloc_fixed if wb == w and ib == i]
            if existing_in_week:
                print(f"  [PRE-ALLOC CONFLICT] '{nm}' already assigned {existing_in_week[0][1]} in week {wk} — skipping {bt}.")
                continue
            model.Add(x[(w, bt, i)] == 1)
            pre_alloc_fixed.add((w, bt, i))
            pre_alloc_slots[slot_key] = nm
            print(f"  [PRE-ALLOC OK] '{nm}' → {bt} week {wk}")
        ...
        for b in BLOCK_TYPES:
            ...
            for i, nm in enumerate(names):
                if any(d in leave.get(nm, set()) for d in days):
                    # Don't conflict with pre-allocations
                    if (w, b, i) not in pre_alloc_fixed:
                        model.Add(x[(w, b, i)] == 0)

    # --- Diagnostic: available weeks per consultant ---
    print(f"\nAvailability (weeks available for ANY block):")
    infeasible_warnings = []
    for i, c in enumerate(consultants):
        available_weeks = 0
        for w, wk in enumerate(weeks):
            ...
            if can_any:
                available_weeks += 1
        min_target = 0 if relax_targets else targets[i]
        status = "OK" if available_weeks >= min_target else "IMPOSSIBLE"
        if status == "IMPOSSIBLE":
            infeasible_warnings.append(f"  ⚠ {c.name}: available_weeks={available_weeks} < min_target={min_target}")
        print(f"  {c.name}: available={available_weeks}/{W} weeks, target={targets[i]}, min_needed={min_target} [{status}]")
    if infeasible_warnings:
        print(f"\n⚠ INFEASIBILITY DETECTED — these consultants cannot meet their targets:")
        for w in infeasible_warnings:
            print(w)
    print()
    ...
    # Exempt consultants whose names contain "Vacancy" OR whose WTE > 1.0
    # (>1 WTE consultants need more allocations and the gap makes it too tight with leave).
    if hard_week_gap:
        exempt_names = {"vacancyc", "vacancyca", "rv vacancy"}
        for i in range(N):
            if names[i].strip().lower() in exempt_names:
                continue
            if consultants[i].wte > 1.0:
                continue  # >1 WTE needs more slots, exempt from gap
            for w in range(W - 1):
    ...
    target_dev_cost = sum(target_deviation_terms) if target_deviation_terms else 0

    model.Minimize(vacancy_cost + diversity_cost + target_dev_cost + 3 * sum(devBH) + 2 * sum(devW) + cardiac_penalty - pref_reward)
    ...
    if status == cp_model.INFEASIBLE:
        print("\n⚠ MODEL IS INFEASIBLE — attempting to identify cause...")
        # Try solving with just mandatory block + one-per-week constraints
        # by checking sufficient statistics
        print(f"  Pre-allocations fixed: {len(pre_alloc_fixed)}")
        for (w, b, i) in sorted(pre_alloc_fixed):
            print(f"    week {weeks[w]} / {b} → {names[i]}")
        # Check if any week has more pre-allocs than available slots
        from collections import Counter
        week_prealloc_count = Counter(w for (w, b, i) in pre_alloc_fixed)
        for w, count in week_prealloc_count.items():
            if count > len(BLOCK_TYPES):
                print(f"  ⚠ Week {weeks[w]} has {count} pre-allocs but only {len(BLOCK_TYPES)} block types!")
        # Check total supply vs total minimum demand
...
def export_to_excel(input_path: str, output_path: str, sol: Dict,
    ...
    # Add header for the Available column
    rota.cell(1, 7).value = "Available"

    # Build unique initials for each consultant
    # First attempt: first letter of first name + first letter of surname
    # If collision: add second letter of surname
    all_cons_names = sorted(cardiac.keys())
    def _make_initials(names_list):
        """Generate unique initials for a list of full names."""
        initials = {}
        ...
        # Resolve collisions: add 2nd letter of surname
        ini_to_names = {}
    ...
    for row_i, d in enumerate(all_days, start=2):
        ...
        # Available consultants: those not on leave/NOC this day
...
def main():
    ...
    # Exit with non-zero code if infeasible so the app tries the next fallback
