...
def solve(
    ...
    def _sc_rule_total(c: Consultant, allowed: List[str]) -> Optional[int]:
        ...
        for rule in c.block_rules:
            ...
            if not eligible:
                continue
            if rule["fractional"]:
                # fractional rules don't contribute a fixed count
                continue
            # Scale rule count to actual cycle length
            scaled = max(1, round(rule["count"] * W / REF_WEEKS))
            total += scaled
        return total if total > 0 else None

    raw_targets = []
    sc_fixed: Dict[int, int] = {}   # consultant index -> SC-derived exact target

    for idx, c in enumerate(consultants):
        fixed = _sc_rule_total(c, allowed_blocks[idx])
        if fixed is not None and c.wte < 1.0:
            # Part-time with explicit rules: target is the rule sum, capped
    ...
    # Adjust rounding errors on non-SC consultants to hit total_slots exactly
    diff = total_slots - sum(targets)
    if diff != 0:
        # Only adjust consultants whose target isn't SC-fixed
    ...
    # --- Each week: exactly one consultant per STANDARD block type ---
    # DMonTue and DThuFri are OPTIONAL half-D blocks (at most 1 per week).
    # They only fire in weeks where a part-time consultant is assigned a half-D.
    ...
    pref_reward_terms = []
    sat_vars: Dict[int, object] = {}   # p_i -> BoolVar
    ...
    for item in pref_items:
        ...
        # sat == 1  iff  at least one matching (week, block) is assigned
    ...
    # Conflict mutual-exclusion: for every pair of preferences from DIFFERENT
    # consultants that share at least one (week, block) variable, the solver
    # can only assign that slot to one person anyway (coverage constraint), but
    # we make it explicit so the higher-weight sat_var is encouraged to win.
    # We do this by penalising the lower-weight sat being 1 when the higher
    # already is — achieved naturally by the reward gap — but we add a hard
    # constraint for same-block same-week to prevent the LP relaxation from
    # over-counting.
    #
    # Group by (week_idx, block_type): at most one preference per slot.
    slot_to_prefs: Dict[tuple, List[int]] = {}
    for item in pref_items:
        for w, wk in enumerate(weeks):
            if _week_overlaps(item["ps"], item["pe"], wk):
                for b in item["blocks"]:
                    key = (w, b)
                    slot_to_prefs.setdefault(key, []).append(item["p_i"])

    for (w, b), p_indices in slot_to_prefs.items():
        if len(p_indices) < 2:
            continue
        # Multiple preferences compete for slot (w, b).
        # Sort by weight descending so we know the priority order.
        competing = sorted(
            [item for item in pref_items if item["p_i"] in p_indices],
            key=lambda it: it["weight"],
            reverse=True,
        )
        # The slot variable x[(w, b, i)] can only be 1 for one consultant.
        # If a lower-weight preference's sat is 1, it must mean no higher-weight
        # preference was also satisfied on this slot — enforced by the objective.
        # Add an explicit: sum of sat_vars for this slot <= 1
        # (Conservative but correct: satisfying one preference blocks all others
        #  for the same slot.)
        slot_sats = [sat_vars[it["p_i"]] for it in competing]
        model.Add(sum(slot_sats) <= 1)
...
def main():
    ...
    try:
        start, end, consultants, leave, bh, prefs = read_inputs(
