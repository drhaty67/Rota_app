...
def solve(
    ...
    # With relax_targets (default), targets are soft — the solver aims for
    # exact targets but can flex ±4 hu (±2 full blocks) when supply is tight.
    # A very high penalty strongly discourages deviation.
    # ===================================================================
    target_slack_vars = []
    ...
    for i, c in enumerate(consultants):
        ...
        if relax_targets:
            ...
            lo = max(0, effective_target - 4)
            hi = effective_target + 4
    ...
    for i, c in enumerate(consultants):
        if c.wte == 1.0 and not c.block_rules:
            # 1 WTE no SC: 5 full blocks total (enforced by C8 target).
            # Diversity penalty (Tier 4) encourages one-of-each.
            # No half-D allowed.
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
            ...
            # < 1 WTE with SC: rules define which types and counts
            for rule in c.block_rules:
            ...
            # Block types NOT mentioned in any rule: disallow
            mentioned = set()
            for rule in c.block_rules:
                for b in rule["blocks"]:
                    mentioned.add(b)
            for bt in BLOCK_TYPES:
                if bt not in mentioned and bt not in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            ...
            # > 1 WTE with SC: base 1 of each mandatory + extras from rules
            for bt in ["AB1", "AB2", "WeekendAB", "WeekendMixed"]:
                if bt in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) >= 1)
            # D-coverage: at least 1 from D-family
            ...
            for rule in c.block_rules:
                rule_blocks = [b for b in rule["blocks"] if b in eligible[i]]
                if not rule_blocks or rule["fractional"]:
                    continue
            ...
            for group_blocks_fs, extra_count in group_extras.items():
                ...
                base_in_group = len([b for b in group_blocks
                                     if b in MANDATORY_BLOCKS and b != "DMonThu"])
            ...
            # < 1 WTE no SC: target enforced by C8, diversity by Tier 4.
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            # Block half-D if not SC-allowed
            ...
            # >= 1 WTE no SC: same as 1 WTE — target enforced by C8,
            # diversity by Tier 4 penalty. No half-D.
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
            for bt in HALF_D_TYPES:
    ...
    for i, c in enumerate(consultants):
        if c.block_rules and c.wte < 1.0:
            continue  # SC consultants have their own exact rules
        for bt in MANDATORY_BLOCKS:
            if bt in eligible[i]:
