...
def solve(
    ...
    # split_d[w]==1 → Split week: 0 DMonThu, exactly 1 DMonTue, exactly 1 DWedThu
    # ===================================================================
    for w in range(W):
        ...
        # If split week: both half-D slots filled, no DMonThu
        model.Add(dmon_thu_sum == 0).OnlyEnforceIf(split_d[w])
        model.Add(dmon_tue_sum == 1).OnlyEnforceIf(split_d[w])
        model.Add(dwed_thu_sum == 1).OnlyEnforceIf(split_d[w])

        # Vacancy not used in split weeks (half-D pairing is hard)
    ...
    for i, c in enumerate(consultants):
        ...
        if is_flex:
            model.Add(hu_alloc == target_half_units[i])
        elif relax_targets:
    ...
    vacancy_cost = VACANCY_PENALTY * sum(
        vacancy[(w, bt)] for w in range(W) for bt in MANDATORY_BLOCKS)
