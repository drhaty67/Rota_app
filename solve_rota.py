...
def _target_allocations(c: Consultant, n_weeks: int = 18) -> Tuple[int, List[str]]:
    ...
    Targets scale with the actual number of weeks in the cycle.
    A 1.0 WTE consultant gets one block per week (n_weeks total).
    Part-time consultants get round(wte * n_weeks) blocks.

    Special circumstances rules (col G) are interpreted as per-cycle counts
    that also scale with n_weeks relative to the reference 18-week cycle:

    < 1.0 WTE with rules:
        The rules define the PATTERN (which block types) but the count per
        rule scales: rule_count * n_weeks / 18, rounded.
        allowed = union of eligible block types across all rules.

    >= 1.0 WTE with rules:
        The rules define EXTRA allocations per 18-week reference cycle.
        Extras scale proportionally with n_weeks.
        target = n_weeks + round(extra_per_18_weeks * n_weeks / 18)

    No rules (col G empty):
        target = round(wte * n_weeks), drop block types from BLOCK_DROP_ORDER.
    """
    REF_WEEKS = 18  # reference cycle length the rules were written for
    ...
    if c.block_rules:
        if c.wte < 1.0:
            # Rules define the pattern; scale counts to actual cycle length
            allowed_set: set = set()
            target = 0
            for rule in c.block_rules:
                ...
                if eligible_in_rule:
                    ...
                    scaled = max(1, round(rule["count"] * n_weeks / REF_WEEKS))
                    target += scaled
            allowed = [b for b in BLOCK_TYPES if b in allowed_set]
            return max(1, target), allowed
        else:
            # Rules are extras on top of the base n_weeks allocations
            extra_target = 0
            ...
            for rule in c.block_rules:
                ...
                if eligible_in_rule:
                    ...
                    scaled_extra = max(1, round(rule["count"] * n_weeks / REF_WEEKS))
                    extra_target += scaled_extra
            ...
            return n_weeks + extra_target, allowed

    # No rules: scale by WTE and cycle length
    target = max(1, round(c.wte * n_weeks))
    if c.wte < 1.0:
        # Start from eligibility-filtered types, then drop from the least-critical
        # end until we reach the right number of types for this WTE.
        n_types = len(DEFAULT_BLOCK_TYPES)  # 5
        allowed = _apply_eligibility(list(DEFAULT_BLOCK_TYPES))
        n_allowed_types = max(1, round(c.wte * n_types))
        # Drop from the least-critical end of what's still eligible
        to_drop = len(allowed) - n_allowed_types
...
def solve(
    ...
    # Per-consultant allocation targets and allowed block types (scaled to W weeks)
    targets = []
    allowed_blocks = []
    for c in consultants:
        t, ab = _target_allocations(c, n_weeks=W)
        targets.append(t)
    ...
    # --- Each week: exactly one consultant per block type ---
    for w in range(W):
    ...
    # -----------------------------------------------------------------------
    # Allocation count constraints
    # -----------------------------------------------------------------------
    # Each consultant gets exactly targets[i] allocations across the cycle.
    # For consultants with block_rules, per-group counts are also enforced.
    # For consultants without rules, we ensure at least 1 of each allowed
    # block type (spread across the cycle) and let the fairness objective
    # distribute the remainder.
    # -----------------------------------------------------------------------
    REF_WEEKS = 18  # reference cycle the rules were written for
    ...
    for i, c in enumerate(consultants):
        ...
        if c.block_rules and c.wte < 1.0:
            # Part-time with explicit rules: enforce scaled per-group counts.
            for rule in c.block_rules:
                eligible_blocks = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible_blocks:
                    continue
                scaled = max(1, round(rule["count"] * W / REF_WEEKS))
                group_alloc = sum(x[(w, b, i)] for w in range(W) for b in eligible_blocks)
                if rule["fractional"]:
                    model.Add(group_alloc <= scaled)
                else:
                    model.Add(group_alloc == scaled)

        elif c.block_rules and c.wte >= 1.0:
            # Full/over-time with rules: at least 1 of each base eligible type,
            # plus scaled extra allocations per rule group.
            for b in DEFAULT_BLOCK_TYPES:
            ...
            for rule in c.block_rules:
                ...
                if not eligible_blocks:
                    continue
                scaled_extra = max(1, round(rule["count"] * W / REF_WEEKS))
                group_alloc = sum(x[(w, b, i)] for w in range(W) for b in eligible_blocks)
                # Must have at least (base 1 + scaled extra) of this group
                model.Add(group_alloc >= 1 + scaled_extra)

        else:
            # No rules: ensure at least 1 of each allowed block type
    ...
    # Preference satisfaction (soft reward)
    ...
    for p_i, p in enumerate(prefs or []):
        ...
        match_vars = [x[(w, b, i)] for w, wk in enumerate(weeks)
                      if _week_overlaps(ps, pe, wk) for b in blocks]
        if not match_vars:
            continue
