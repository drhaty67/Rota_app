...
#   DThuFri  = D shift covering Thu + Fri only
# The solver picks one of these two per cycle for the consultant.
# ---------------------------------------------------------------------------

BLOCK_TYPES = ["AB1", "AB2", "DMonThu", "DMonTue", "DThuFri", "WeekendAB", "WeekendMixed"]
# Drop order for <1 WTE consultants with NO special_circumstances rules.
# Half-D types (DMonTue, DThuFri) are never in the default allowed set —
...
D_BLOCK_TYPES = {"DMonThu", "DMonTue", "DThuFri"}
...
def _target_allocations(c: Consultant, n_weeks: int = 18) -> Tuple[int, List[str]]:
    ...
    def _apply_eligibility(allowed: List[str]) -> List[str]:
        ...
        if not c.eligible_d or c.no_d:
            for bt in ("DMonThu", "DMonTue", "DThuFri", "WeekendMixed"):
                if bt in result:
                    result.remove(bt)
        if c.no_weekend:
...
def solve(
    ...
    # -----------------------------------------------------------------------
    # Compute WTE-proportional targets
    # -----------------------------------------------------------------------
    # Total block-slots in the cycle = W * 5 (5 block types per week).
    # Each consultant's fair share = (their WTE / total WTE) * W * 5.
    # For over/under WTE consultants, WTE already encodes extra duty.
    # The 1-week-gap constraint caps any consultant at ceil(W/2) allocations.
    #
    # For consultants with explicit SC rules, their target is the sum of
    # rule counts (scaled to W) rather than the raw WTE share, so that
    # the total-count and per-group-count constraints are always consistent.
    # -----------------------------------------------------------------------
    total_wte = sum(c.wte for c in consultants)
    total_slots = W * 5  # mandatory blocks only (AB1+AB2+DMonThu+WeekendAB+WeekendMixed)
    max_per_consultant = (W + 1) // 2  # hard ceiling from 1-week-gap constraint
    REF_WEEKS = 18  # the cycle length the SC rules were written for
    MANDATORY_BLOCKS = ["AB1", "AB2", "DMonThu", "WeekendAB", "WeekendMixed"]
    OPTIONAL_BLOCKS  = ["DMonTue", "DThuFri"]

    def _sc_rule_total(c: Consultant, allowed: List[str]) -> Optional[int]:
        """Return the exact target implied by SC rules, or None if no rules.
        Counts ALL rule allocations including optional (half-D) blocks."""
        if not c.block_rules:
            return None
        total = 0
        for rule in c.block_rules:
            eligible = [b for b in rule["blocks"] if b in allowed]
            if not eligible:
                continue
            if rule["fractional"]:
                continue
    ...
    def _sc_mandatory_count(c: Consultant, allowed: List[str]) -> int:
        """Count how many of a SC consultant's allocations fill MANDATORY slots.
        DMonTue/DThuFri are optional — they don't consume mandatory slots."""
        if not c.block_rules:
            return 0
        total = 0
        for rule in c.block_rules:
            ...
            if not eligible or rule["fractional"]:
                continue
            # Does this rule exclusively target optional blocks?
            mandatory_eligible = [b for b in eligible if b in MANDATORY_BLOCKS]
            if mandatory_eligible:
                # This rule fills mandatory slots
                total += max(1, round(rule["count"] * W / REF_WEEKS))
            # else: rule targets only optional blocks — doesn't consume mandatory capacity
        return total

    raw_targets = []
    sc_fixed: Dict[int, int] = {}

    for idx, c in enumerate(consultants):
        fixed = _sc_rule_total(c, allowed_blocks[idx])
        if fixed is not None and c.wte < 1.0:
            t = max(1, min(max_per_consultant, fixed))
            sc_fixed[idx] = t
            raw_targets.append(float(t))
        else:
            share = (c.wte / total_wte) * total_slots
            raw_targets.append(share)

    # Round to integers while preserving total as closely as possible.
    # SC-fixed consultants that have optional-only rules (half-D) consume
    # fewer mandatory slots than their target — account for this in the budget.
    sc_mandatory_contribution = 0
    sc_total_contribution = 0
    for idx in sc_fixed:
        sc_total_contribution += sc_fixed[idx]
        sc_mandatory_contribution += _sc_mandatory_count(
            consultants[idx], allowed_blocks[idx])

    # The remaining non-SC consultants must fill:
    #   total_slots (mandatory) - sc_mandatory_contribution
    remaining_mandatory = total_slots - sc_mandatory_contribution

    targets = []
    for idx in range(N):
        if idx in sc_fixed:
            targets.append(sc_fixed[idx])
        else:
            targets.append(max(1, min(max_per_consultant, round(raw_targets[idx]))))

    # Adjust non-SC targets to sum to remaining_mandatory (the mandatory slots they must fill)
    non_sc_sum = sum(targets[i] for i in range(N) if i not in sc_fixed)
    diff = remaining_mandatory - non_sc_sum
    if diff != 0:
        free_idx = [i for i in range(N) if i not in sc_fixed]
        remainders = [(raw_targets[i] - targets[i], i) for i in free_idx]
        remainders.sort(reverse=(diff > 0))
        for _, i in remainders[:abs(diff)]:
            adj = 1 if diff > 0 else -1
            new_t = targets[i] + adj
            if 1 <= new_t <= max_per_consultant:
                targets[i] = new_t
                diff -= adj
            if diff == 0:
                break
    ...
    # --- Each week: exactly one consultant per MANDATORY block type ---
    # DMonTue and DThuFri are OPTIONAL half-D blocks (at most 1 per week).
    for w in range(W):
    ...
    # These are admin-specified shifts that must appear in the solution.
    # The solver will NOT count them towards the consultant's target — instead
    # the target is reduced by the number of pre-allocated blocks for that person.
    pre_alloc_fixed: Set[Tuple[int, str, int]] = set()  # (week_idx, block_type, consultant_idx)
    ...
    if pre_allocations:
        ...
        # Reduce targets for consultants with pre-allocations
        pre_alloc_per_consultant: Dict[int, int] = {}
        for (w, bt, i) in pre_alloc_fixed:
            pre_alloc_per_consultant[i] = pre_alloc_per_consultant.get(i, 0) + 1

        for i, count in pre_alloc_per_consultant.items():
            targets[i] = max(1, targets[i])  # target stays the same — pre-allocs count toward it
            # Actually: the target already includes this person's fair share.
            # The pre-allocation IS one of their target allocations, so we
            # don't reduce the target — the x[(w,b,i)]==1 forces it to be
            # counted in total_alloc==targets[i] naturally.
    ...
    # -----------------------------------------------------------------------
    # Allocation count + SC rule enforcement constraints
    # -----------------------------------------------------------------------
    # (1) Hard total: each consultant gets exactly targets[i] allocations.
    # (2) Hard per-rule: for consultants with explicit SC rules, each rule's
    #     block group must be satisfied the required number of times.
    #     This is what makes "1 DMonTue or DThuFri" actually happen.
    #
    #     < 1.0 WTE with rules:
    #       Each non-fractional rule:  sum(x[w,b,i] for w,b in rule) == count
    #       Each fractional rule:      sum(x[w,b,i] for w,b in rule) <= count
    #     >= 1.0 WTE with rules (extras on top of base):
    #       For each rule:  sum(x[w,b,i] for w,b in rule) >= 1
    #       (at least one allocation from each rule's group)
    # -----------------------------------------------------------------------
    for i, c in enumerate(consultants):
        ...
        if c.wte < 1.0:
            # Each rule defines the MINIMUM number from that block group.
            # Using >= (not ==) gives the solver flexibility to satisfy the
            # global mandatory-slot and 1-week-gap constraints simultaneously,
            # while still guaranteeing the stipulated block types are assigned.
            for rule in c.block_rules:
            ...
            # >= 1 WTE: rules describe extras; each group must appear at least
            # the required number of times (scaled to actual cycle length)
            for rule in c.block_rules:
                eligible = [b for b in rule["blocks"] if b in allowed_blocks[i]]
                if not eligible:
                    continue
    ...
    #   Wed (2): DMonThu
    #   Thu (3): DMonThu, DThuFri
    #   Fri (4): WeekendMixed, DThuFri
    # -----------------------------------------------------------------------
    ...
    _D_blocks_for_day = {
        ...
        2: ["DMonThu"],                     # Wed
        3: ["DMonThu", "DThuFri"],          # Thu
        4: ["WeekendMixed", "DThuFri"],     # Fri
    }
    ...
    sum_targets = sum(targets) if sum(targets) > 0 else 1
    SCALE = 1000
    expected_bh = [int(round(bh_all * (targets[i] / sum_targets) * SCALE)) for i in range(N)]
    ...
    weekend_all = 2 * W
    expected_w = [int(round(weekend_all * (targets[i] / sum_targets) * SCALE)) for i in range(N)]
    ...
    #   D       -> DMonThu, DMonTue, DThuFri  (any D block)
    ...
    def _shift_to_blocks(st: str) -> List[str]:
        ...
        if st_up == "B":
            return ["AB2"]
        if st_up == "D":
            return ["DMonThu", "DMonTue", "DThuFri"]
    ...
    model.Minimize(3 * sum(devBH) + 2 * sum(devW) + cardiac_penalty - pref_reward)
...
def export_to_excel(input_path: str, output_path: str, sol: Dict,
    ...
    for r_i, wk in enumerate(weeks, start=2):
        ...
        wa.cell(r_i, 6).value = sol["assignments"][wk].get("DThuFri", "")
    ...
    for row_i, d in enumerate(all_days, start=2):
        ...
        # DMonThu covers Mon-Thu; DMonTue covers Mon-Tue; DThuFri covers Thu-Fri.
        ...
        if dow == 0 or dow == 1:    # Mon, Tue
            ...
            D = asg.get("DMonThu") or asg.get("DThuFri") or ""
        elif dow == 4:              # Fri
            D = asg.get("WeekendMixed") or asg.get("DThuFri") or ""
        else:
            D = ""
