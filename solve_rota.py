...
def solve(
    ...
    FLEX_NAMES = {c.name.strip().lower() for c in consultants
                  if "vacancy" in c.name.strip().lower()}

    # Half-block unit constants
    FULL_BLOCK_TYPES = ["AB1", "AB2", "DMonThu", "WeekendAB", "WeekendMixed"]
    HALF_D_TYPES = ["DMonTue", "DWedThu"]

    print(f"\n{'='*60}")
    print(f"SOLVER DIAGNOSTICS (half-D = 0.5 model)")
    print(f"{'='*60}")
    ...
    # -----------------------------------------------------------------------
    # Per-consultant: eligible blocks, target (in half-block units), availability
    # -----------------------------------------------------------------------
    ...
    for i, c in enumerate(consultants):
        if c.wte == 1.0 and not c.block_rules:
            ...
            # < 1 WTE with SC: sum rules, half-D counts as 1, full counts as 2
            total_hu = 0
            for rule in c.block_rules:
                if rule["fractional"]:
                    total_hu += 1
            ...
            # > 1 WTE with SC: base 10 (5 full blocks) + extras from SC
            base_hu = 10
            extras_hu = 0
            for rule in c.block_rules:
                if rule["fractional"]:
            ...
            # < 1 WTE no SC: round(WTE * 5) full blocks * 2
    ...
    # Max half-units a consultant can earn given availability
    max_possible_hu: List[int] = []
    for i, c in enumerate(consultants):
        max_possible_hu.append(avail_per_consultant[i] * 2)
    ...
    for i, c in enumerate(consultants):
        ...
        flag = ""
        if not is_flex and max_possible_hu[i] < target_half_units[i]:
            flag = " *** IMPOSSIBLE ***"
    ...
    if infeasible_flags:
        print(f"\n*** WARNING: These consultants cannot meet their targets: "
              f"{infeasible_flags}")
        print(f"    Their max possible half-units are less than their target.")
    ...
    vacancy: Dict[Tuple[int, str], any] = {}
    NON_D_MANDATORY = [bt for bt in MANDATORY_BLOCKS if bt != "DMonThu"]
    for w in range(W):
        for bt in NON_D_MANDATORY:
            vacancy[(w, bt)] = model.NewBoolVar(f"vac_{w}_{bt}")
        # DMonThu vacancy (used when neither full-D nor split-D fills the week)
        vacancy[(w, "DMonThu")] = model.NewBoolVar(f"vac_{w}_DMonThu")

    # split_d[w]: binary — is week w a split-D week?
    split_d: Dict[int, any] = {}
    for w in range(W):
        split_d[w] = model.NewBoolVar(f"split_d_{w}")

    # Collect soft penalty terms
    soft_penalty_terms: List = []

    # ===================================================================
    # HARD CONSTRAINT 1: Non-D mandatory blocks — exactly 1 consultant OR vacancy
    # ===================================================================
    for w in range(W):
        for bt in NON_D_MANDATORY:
            model.Add(
    ...
    # ===================================================================
    # HARD CONSTRAINT 2: D-Coverage via split_d variable
    # split_d[w]==0 → DMonThu week: exactly 1 DMonThu OR vacancy, 0 half-D
    # split_d[w]==1 → Split week: 0 DMonThu, at least 1 half-D assigned
    #                 (both DMonTue and DWedThu ideally filled, but vacancy
    #                  covers if no SC-allowed consultant is available)
    # ===================================================================
    # Vacancy variables for half-D slots in split weeks
    vacancy_dmt: Dict[int, any] = {}
    vacancy_dwt: Dict[int, any] = {}
    for w in range(W):
        vacancy_dmt[w] = model.NewBoolVar(f"vac_{w}_DMonTue")
        vacancy_dwt[w] = model.NewBoolVar(f"vac_{w}_DWedThu")

    for w in range(W):
        dmon_thu_sum = sum(x[(w, "DMonThu", i)] for i in range(N))
        dmon_tue_sum = sum(x[(w, "DMonTue", i)] for i in range(N))
        dwed_thu_sum = sum(x[(w, "DWedThu", i)] for i in range(N))

        # If NOT split week: DMonThu assigned or vacancy
        model.Add(dmon_thu_sum + vacancy[(w, "DMonThu")] == 1).OnlyEnforceIf(
            split_d[w].Not())
        model.Add(dmon_tue_sum == 0).OnlyEnforceIf(split_d[w].Not())
        model.Add(dwed_thu_sum == 0).OnlyEnforceIf(split_d[w].Not())

        # If split week: no DMonThu, each half-D has exactly 1 person or vacancy
        model.Add(dmon_thu_sum == 0).OnlyEnforceIf(split_d[w])
        model.Add(vacancy[(w, "DMonThu")] == 0).OnlyEnforceIf(split_d[w])
        model.Add(dmon_tue_sum + vacancy_dmt[w] == 1).OnlyEnforceIf(split_d[w])
        model.Add(dwed_thu_sum + vacancy_dwt[w] == 1).OnlyEnforceIf(split_d[w])

        # In non-split weeks, half-D vacancy vars must be 0
        model.Add(vacancy_dmt[w] == 0).OnlyEnforceIf(split_d[w].Not())
        model.Add(vacancy_dwt[w] == 0).OnlyEnforceIf(split_d[w].Not())
    ...
    # ===================================================================
    # HARD CONSTRAINT 6: DMonTue/DWedThu restricted to SC half-D consultants
    # Only consultants whose SC rules include DMonTue/DWedThu (plus flex/vacancy)
    # ===================================================================
    for w in range(W):
    ...
    # If a half-D block is pre-allocated, force split_d for that week.
    # ===================================================================
    ...
    if pre_allocations:
        ...
        for pa in pre_allocations:
            ...
            # If half-D block pre-allocated, force split_d for that week
            if bt in HALF_D_TYPES:
                model.Add(split_d[w] == 1)
            # If DMonThu pre-allocated, force non-split for that week
            if bt == "DMonThu":
                model.Add(split_d[w] == 0)
            print(f"  [OK] '{nm}' -> {bt} week {wk}")
        print()

    # ===================================================================
    # HARD CONSTRAINT 8: Allocation targets (half-block units)
    # Each consultant's weighted allocation must match target_half_units.
    # Full blocks contribute 2 half-units, half-D blocks contribute 1.
    # ===================================================================
    target_slack_vars = []
    ...
    for i, c in enumerate(consultants):
        ...
        if is_flex:
            # Flex/vacancy: target is preferred but allow some slack
            model.Add(hu_alloc <= target_half_units[i] + 2)
            # Don't force minimum for flex - let the solver use them as needed
        elif relax_targets:
            effective_target = min(target_half_units[i], max_possible_hu[i])
            lo = max(0, effective_target - 2)  # allow +/- 1 full block = 2 hu
            ...
            effective_target = target_half_units[i]
    ...
    for i, c in enumerate(consultants):
        ...
        if c.wte == 1.0 and not c.block_rules:
            # 1 WTE no SC: one each of AB1, AB2, WeekendAB, WeekendMixed, DMonThu
            for bt in ["AB1", "AB2", "WeekendAB", "WeekendMixed"]:
                if bt in eligible[i]:
                    if relax_targets:
            ...
            if "DMonThu" in eligible[i]:
                if relax_targets:
                    ...
                    model.Add(sum(x[(w, "DMonThu", i)] for w in range(W)) == 1)
            # Block half-D for 1 WTE non-SC
            model.Add(sum(x[(w, "DMonTue", i)] for w in range(W)) == 0)
            model.Add(sum(x[(w, "DWedThu", i)] for w in range(W)) == 0)
            ...
            # > 1 WTE with SC: base mandatory + extras from rules
            # For non-D mandatory blocks, require >= 1 each (or soft if relaxed)
            if not relax_targets:
                for bt in MANDATORY_BLOCKS:
                    if bt == "DMonThu":
                        continue  # D-coverage handled separately below
                    if bt in eligible[i]:
                        model.Add(sum(x[(w, bt, i)] for w in range(W)) >= 1)
            # D-coverage: at least 1 from the D-family (DMonThu OR half-D)
            d_family = [b for b in ["DMonThu", "DMonTue", "DWedThu"]
                        if b in eligible[i]]
            if d_family and not relax_targets:
            ...
            for group_blocks_fs, extra_count in group_extras.items():
                ...
                # Don't double-count DMonThu base (handled by D-family above)
                base_in_group = len([b for b in group_blocks
                                     if b in MANDATORY_BLOCKS
                                     and b != "DMonThu"])
                if relax_targets:
                    # Soft: allow up to base + extra, don't force minimum
                    pass
                else:
                    model.Add(group_sum >= base_in_group + extra_count)

        elif c.wte < 1.0 and not c.block_rules:
            # < 1 WTE no SC: no duplicates if possible
            eligible_types = [bt for bt in BLOCK_TYPES if bt in eligible[i]]
            n_eligible = len(eligible_types)
            target_blocks = max(1, round(c.wte * 5))
            for bt in BLOCK_TYPES:
                if bt not in eligible[i]:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) == 0)
                elif n_eligible >= target_blocks:
                    model.Add(sum(x[(w, bt, i)] for w in range(W)) <= 1)
                else:
            ...
            # Block half-D for non-SC consultants
            if i not in sc_allows_half_d:
                model.Add(sum(x[(w, "DMonTue", i)] for w in range(W)) == 0)
                model.Add(sum(x[(w, "DWedThu", i)] for w in range(W)) == 0)
    ...
    # ===================================================================
    vacancy_cost = VACANCY_PENALTY * (
        sum(vacancy[(w, bt)] for w in range(W) for bt in MANDATORY_BLOCKS)
        + sum(vacancy_dmt[w] + vacancy_dwt[w] for w in range(W))
    )

    target_dev_cost = (TARGET_DEVIATION_PENALTY * sum(target_slack_vars)
                       if target_slack_vars else 0)

    # Flex consultant under-use penalty
    flex_underuse_terms = []
    for i, c in enumerate(consultants):
        if c.name.strip().lower() in FLEX_NAMES:
            hu_alloc = (
                sum(x[(w, bt, i)] * 2 for w in range(W)
                    for bt in FULL_BLOCK_TYPES)
                + sum(x[(w, bt, i)] * 1 for w in range(W)
                      for bt in HALF_D_TYPES)
            )
            shortfall = model.NewIntVar(0, W * 2, f"flex_short_{i}")
            ideal = min(target_half_units[i], max_possible_hu[i])
            model.Add(shortfall >= ideal - hu_alloc)
    ...
    if status == cp_model.INFEASIBLE:
        ...
        total_min_demand = sum(
            0 if (relax_targets or c.name.strip().lower() in FLEX_NAMES)
            else target_half_units[idx]
            for idx, c in enumerate(consultants)
        )
        total_supply = W * len(BLOCK_TYPES) * 2  # in half-units
        print(f"  Total supply (half-units): {total_supply}")
        print(f"  Total hard demand (half-units): {total_min_demand}")
