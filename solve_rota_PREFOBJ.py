...
def solve(start: date, end: date, consultants: List[Consultant], leave: Dict[str, Set[date]], bank_holidays: Set[date],
    ...
    # Sheet: preferred_shifts with columns:
    #   name | start_date | end_date | kind(prefer/avoid) | shift_type | weight | notes
    # shift_type supported (block-level): AB1, AB2, DMonThu, WeekendAB, WeekendMixed, AB, Weekend
    # -----------------------------
    pref_items = []
    try:
        if "preferred_shifts" in wb.sheetnames:
            ...
            for row in pws.iter_rows(min_row=2, values_only=True):
                if not row or not row[0]:
                    continue
                name, sd, ed, kind, stype, weight, notes = (list(row) + [None]*7)[:7]
                name = str(name).strip()
                ...
                if not sd or not ed:
                    continue
                kind_s = str(kind).strip().lower() if kind is not None else "prefer"
                ...
                is_avoid = any(k in kind_s for k in ["avoid", "no", "dont", "do not", "can't", "cannot"])
                sign = -1 if is_avoid else 1

                # Map shift types to block types
                blocks = []
                if stype_s in block_types:
                    blocks = [stype_s]
                else:
                    st_lower = stype_s.lower()
                    if st_lower in ["ab", "a/b", "ab1+ab2", "weekday_ab"]:
                        blocks = ["AB1", "AB2"]
